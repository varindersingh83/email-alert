...
- This repo sends messages only. It does not read a mailbox or trigger on incoming email; connect an approved email automation to `POST /send` separately. For client-owned Gmail workflows built in Activepieces, see [the Activepieces Gmail workflow playbook](docs/activepieces-gmail-workflows.md) and [the SIT bootstrap record](verification/activepieces-sit-bootstrap.md).
